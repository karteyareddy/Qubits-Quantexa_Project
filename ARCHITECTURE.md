...
## Stage 15 Next.js Interactive Traffic Dashboard
Stage 15 builds the complete Next.js frontend dashboard (`frontend/`), exposing an urban traffic control center UI. It includes typed API (`lib/api.ts`) and WebSocket (`lib/websocket.ts`) client adapters, custom React state hooks (`hooks/useDashboard.ts`), an interactive 6-intersection SVG traffic map (`TrafficMap.tsx`), live metric cards and trend sparklines (`MetricCards.tsx`, `MetricTrends.tsx`), QAOA hybrid optimization and classical reference panels (`QuantumPanel.tsx`, `ClassicalComparison.tsx`), emergency green corridor route & window details (`EmergencyCorridorPanel.tsx`), and dynamic event injection modals (`EventInjectionModal.tsx`).
