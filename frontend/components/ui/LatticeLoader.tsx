...
const LIT: Record<number, string> = {
  62: 'll-lit-62 animate-[lattice-on_var(--ll-cycle)_infinite]',
  45: 'll-lit-45 animate-[lattice-on-45_var(--ll-cycle)_infinite]',
  35: 'll-lit-35 animate-[lattice-on-35_var(--ll-cycle)_infinite]',
  25: 'll-lit-25 animate-[lattice-on-25_var(--ll-cycle)_infinite]'
};
const STYLE = `
.ll-root {
  display: inline-flex;
...
.ll-cell:not([data-hole]) {
  opacity: var(--ll-idle);
  animation-timing-function: var(--ll-ease-in-out);
}
.ll-root[data-glow] .ll-cell:not([data-hole]) {
  box-shadow: 0 0 calc(var(--ll-cell) * 1.2) calc(var(--ll-cell) * 0.12) var(--ll-glow);
}
.ll-cell.ll-lit-62 { animation: lattice-on var(--ll-cycle) infinite; }
.ll-cell.ll-lit-45 { animation: lattice-on-45 var(--ll-cycle) infinite; }
.ll-cell.ll-lit-35 { animation: lattice-on-35 var(--ll-cycle) infinite; }
.ll-cell.ll-lit-25 { animation: lattice-on-25 var(--ll-cycle) infinite; }

.ll-mark .ll-cell {
  opacity: var(--ll-idle);
  transition: opacity 200ms ease, background-color 200ms ease;
}
.ll-mark .ll-cell[data-on] {
  background: var(--ll-mark);
  opacity: var(--ll-peak);
}
.ll-root[data-glow] .ll-mark .ll-cell[data-on] {
...
export const LatticeLoader: React.FC<LatticeLoaderProps> = ({
  ...
  return (
    ...
    >
      <style>{STYLE}</style>
      <span className="ll-grid-wrap grid shrink-0" aria-hidden="true">
        ...
        <span className="ll-mark [grid-area:1/1] grid [grid-template-columns:repeat(var(--ll-n),var(--ll-cell))] [gap:var(--ll-gap)] origin-center opacity-0 [transform:scale(0.9)] [transition:opacity_160ms_var(--ll-ease-out),transform_160ms_var(--ll-ease-out)] group-data-[status=done]:opacity-100 group-data-[status=done]:[transform:none] group-data-[status=done]:[transition:opacity_200ms_ease,transform_200ms_var(--ll-ease-out)] group-data-[status=error]:opacity-100 group-data-[status=error]:[transform:none] group-data-[status=error]:[transition:opacity_200ms_ease,transform_200ms_var(--ll-ease-out)]">
          {pat.cells.map((_, i) => (
            <span
              key={i}
              className={`ll-cell ${CELL} [opacity:var(--ll-idle)] [transition:opacity_200ms_ease,background-color_200ms_ease] data-[on]:[background:var(--ll-mark)] data-[on]:[opacity:var(--ll-peak)] group-data-[glow]:data-[on]:[box-shadow:0_0_calc(var(--ll-cell)*1.2)_calc(var(--ll-cell)*0.12)_var(--ll-mark-glow)]`}
              data-on={marks[mark].includes(i) ? '' : undefined}
            />
          ))}
        </span>
      </span>
      <span className="ll-label-container relative inline-block font-medium" aria-hidden="true">
        <span
      ...
      {showTimer ? (
        <span
          ref={timerRef}
          className="ll-timer font-mono tabular-nums opacity-60 [font-size:calc(var(--ll-font)*0.875)]"
          aria-hidden="true"
        >
          0.0s
        </span>
