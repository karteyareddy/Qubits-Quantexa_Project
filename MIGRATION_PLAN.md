...
| `legacy/solver.py` | **Refactor**, then **Deprecate** | Useful adapter behavior moves to `backend/app/optimization/solvers/classical.py`; original goes to archive/history | The dispatch pattern and backend concepts are useful, but the module duplicates active code and hides fallback state. | Solver protocol/result model, fallback tests | Adapter extraction in Stages 7 and 9-10; archive after Stage 10 |
...
| Existing file | Action | Final location | Reason | Dependencies | Migration order |
|---|---|---|---|---|---|
| `requirements.txt` | **Replace** | `backend/requirements.txt` | Current file lists future backend libraries but cannot run the current app. Final backend dependencies must be validated and pinned. | Backend skeleton and clean install validation | Stage 1 creates backend manifest; pin Stage 17 |
